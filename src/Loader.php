 ...
 * - parsing lines that look shell include file, e.g `source /data/demo.ini`, `source demo.ini`.
 */
class Loader
{
    /**
    ...
    public function load()
    {
        ...
        return $this->loadData($this->filePath);
    }
    ...
    protected function loadData($filePath)
    {
        ...
        foreach ($lines as $line) {
            ...
            } elseif ($filePath = $this->isSource($line, $filePath)) {
                $lines[$line] = $this->loadData($filePath);
            }
        }
     ...
     */
    protected function isSource($line, $filePath = '')
    {
        if (stripos($line, 'source ') === 0 && $file = substr($line, 7)) {
            if (!is_file($file)) {
                $filePath = $filePath ?: $this->filePath;
                $file = dirname($filePath) . '/' . trim($file, '/');
